...
GMT_COR_LOAD ## Core Loading with Bundle Types
 ...
 5 10*1 0*6  5   ## nb = 1  1-ТВС центральной зоны      
 5 10*2 0*7  5   ## nb = 2  2-ТВС переферийной зоны     
...
%for i in range(7):
%if i!=4:
${'%-13s'%D[0].split()[0]}      ${'%-13s'%sabs[0].split()[0]}               ${'%-13s'%nusfis[0].split()[0]}                 ${'%13s'%sfis[0].split()[0]}
${'%-13s'%D[0].split()[1]}      ${'%-13s'%sabs[0].split()[1]}               ${'%-13s'%nusfis[0].split()[1]}                 ${'%13s'%sfis[0].split()[1]}

${'%-13s'%matr[0][0]}    ${'%-13s'%matr[0][1]}  
${'%-13s'%matr[0][2]}    ${'%-13s'%matr[0][3]}                ${i+1}               ${keff[0]}           0.0
%endif
%if i==4:
${'%-13s'%D[0].split()[0]}      ${'%-13s'%sabs[0].split()[0]}               0.00000000000                 0.00000000000
${'%-13s'%D[0].split()[1]}      ${'%-13s'%sabs[0].split()[1]}               0.00000000000                 0.00000000000

${'%-13s'%matr[1][0]}    ${'%-13s'%matr[1][1]}  
${'%-13s'%matr[1][2]}    ${'%-13s'%matr[1][3]}                ${i+1}               ${keff[1]}           0.0
%endif
%endfor
...
matr = []
keff = []
file = yaml.load(open('burn_sketch.yaml'))
i=8
...
print(pechat)
data = open(r'C:\Users\delez\Desktop\DIPLOM\kp\sketch_fast_reactor\Input\burn.dat', 'w')
data.write(pechat)
data.close()
